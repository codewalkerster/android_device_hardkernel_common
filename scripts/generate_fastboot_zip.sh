...
if [ "$TARGET_NAME" = "signed" ]; then
	...
	(cd $TARGET_NAME-fastboot; zip -1 -r ../out_publish/$BOARD_NAME-fastboot *)
fi
...
./out/host/linux-x86/bin/mkbootfs -d ${TARGET_NAME}_target/SYSTEM ${TARGET_NAME}_target/VENDOR_BOOT/RAMDISK out/target/product/$BOARD_NAME/vendor_debug_ramdisk out/target/product/$BOARD_NAME/debug_ramdisk | out/host/linux-x86/bin/lz4 -l -12 --favor-decSpeed > ${TARGET_NAME}_target/vendor_ramdisk-debug.cpio.lz4
...
if [ "$TARGET_NAME" = "signed" ]; then
	...
	./out/host/linux-x86/bin/mkbootimg --dtb ${TARGET_NAME}_target/VENDOR_BOOT/dtb --base 0x0 --vendor_cmdline "bootconfig bootconfig" --vendor_bootconfig ${TARGET_NAME}_target/VENDOR_BOOT/vendor_bootconfig --kernel_offset 0x2080000 --header_version 4 --vendor_ramdisk ${TARGET_NAME}_target/vendor_ramdisk-debug.cpio.lz4 --ramdisk_type RECOVERY --ramdisk_name recovery --vendor_ramdisk_fragment out/target/product/$BOARD_NAME/obj/PACKAGING/vendor_ramdisk_fragments_intermediates/recovery.cpio.lz4 --vendor_boot out_publish/vendor_boot-debug.img
