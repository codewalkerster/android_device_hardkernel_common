...
PRODUCT_COPY_FILES += $(foreach tuner, $(TUNER_MODULE),\
	...
	ifeq ($(TARGET_BUILD_KERNEL_USING_14_5.15), true)
		PRODUCT_COPY_FILES += $(foreach tuner, $(TUNER_MODULE),\
			$(if $(findstring true, $(KERNEL_A32_SUPPORT)),\
				device/amlogic/common/tuner/14_5.15/32_5_15/$(tuner)_fe_32.ko:$(PRODUCT_OUT)/obj/lib_vendor/$(tuner)_fe.ko,\
				device/amlogic/common/tuner/14_5.15/64_5_15/$(tuner)_fe_64.ko:$(PRODUCT_OUT)/obj/lib_vendor/$(tuner)_fe.ko)\
				device/amlogic/common/initscripts/tuner/$(tuner)_fe.rc:$(TARGET_COPY_OUT_VENDOR)/etc/init/$(tuner)_fe.rc)
	else
		PRODUCT_COPY_FILES += $(foreach tuner, $(TUNER_MODULE),\
			$(if $(findstring true, $(KERNEL_A32_SUPPORT)),\
				device/amlogic/common/tuner/13_5.15/32_5_15/$(tuner)_fe_32.ko:$(PRODUCT_OUT)/obj/lib_vendor/$(tuner)_fe.ko,\
				device/amlogic/common/tuner/13_5.15/64_5_15/$(tuner)_fe_64.ko:$(PRODUCT_OUT)/obj/lib_vendor/$(tuner)_fe.ko)\
				device/amlogic/common/initscripts/tuner/$(tuner)_fe.rc:$(TARGET_COPY_OUT_VENDOR)/etc/init/$(tuner)_fe.rc)
	endif
else
PRODUCT_COPY_FILES += $(foreach tuner, $(TUNER_MODULE),\
